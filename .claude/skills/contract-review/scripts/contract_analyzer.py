...
class ContractAnalyzer:
    ...
    def extract_text(self) -> str:
        ...
        try:
            ...
            print(f"⚠️  pandoc提取失败: {e}，改用docx回退提取")
            self.full_text = self._extract_text_fallback_docx()
            return self.full_text
        except subprocess.TimeoutExpired:
            print(f"⚠️  pandoc提取超时，改用docx回退提取")
            self.full_text = self._extract_text_fallback_docx()
            return self.full_text
        except FileNotFoundError:
            print(f"⚠️  未找到pandoc命令,改用docx回退提取")
            self.full_text = self._extract_text_fallback_docx()
            return self.full_text

    def _extract_text_fallback_docx(self) -> str:
        """
        pandoc不可用时: 直接从 docx 的 word/document.xml 抽取每段落文本。

        注意: 仅用于智能搜索关键词/摘要定位，不改变原合同内容。
        """
        try:
